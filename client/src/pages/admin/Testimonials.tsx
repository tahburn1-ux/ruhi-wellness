    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-8 w-full max-w-lg">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-serif text-xl font-bold text-[oklch(0.18_0.05_60)]">{initial ? "Edit Testimonial" : "Add Testimonial"}</h2>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-[oklch(0.50_0.03_65)] mb-1">Rating</label>
            <div className="flex gap-2">
              {[1,2,3,4,5].map(n => (
                <button key={n} onClick={() => setForm({ ...form, rating: n })}>
                  <Star className={`w-6 h-6 transition-colors ${n <= form.rating ? "fill-yellow-400 text-yellow-400" : "text-[oklch(0.80_0.02_78)]"}`} />
                </button>
              ))}
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-[oklch(0.50_0.03_65)] mb-1">Testimonial Text *</label>
            <textarea value={form.text} onChange={e => setForm({ ...form, text: e.target.value })}
              rows={4} className="w-full border border-[oklch(0.87_0.025_78)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[oklch(0.52_0.10_75)] resize-none" />
          </div>
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm text-[oklch(0.35_0.04_65)]">Show on website</span>
          </label>
        </div>
  ...
  return (
    <div className="flex min-h-screen bg-[oklch(0.97_0.01_210)]">
      ...
      <main className="flex-1 p-8 overflow-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="font-serif text-3xl font-bold text-[oklch(0.18_0.05_60)]">Testimonials</h1>
            <p className="text-[oklch(0.50_0.03_65)] mt-1">Manage client reviews shown on the website</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {(testimonials as any[]).map((t: any) => (
            <div key={t.id} className="glass-card rounded-2xl p-5">
              ...
              <p className="text-sm text-[oklch(0.35_0.04_65)] italic mb-4 line-clamp-3">"{t.text}"</p>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[oklch(0.52_0.10_75)] to-[oklch(0.52_0.10_75)] flex items-center justify-center text-white text-xs font-bold">
                    {t.initials || t.clientName.slice(0,2).toUpperCase()}
                  </div>
                  <div>
                    <div className="text-sm font-semibold text-[oklch(0.22_0.05_60)]">{t.clientName}</div>
                    {t.serviceName && <div className="text-xs text-[oklch(0.52_0.10_75)]">{t.serviceName}</div>}
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setEditing(t)} className="p-1.5 rounded-lg hover:bg-[oklch(0.92_0.03_78)] text-[oklch(0.52_0.10_75)]">

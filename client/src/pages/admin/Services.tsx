...
function ServiceForm({ initial, onSave, onCancel }: { initial?: any; onSave: (data: any) => void; onCancel: () => void }) {
  const [form, setForm] = useState(initial || {
    ...
    duration: "45 Min Session", description: "", icon: "💧",
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 overflow-auto">
      <div className="bg-white rounded-2xl p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-serif text-2xl font-bold text-[oklch(0.18_0.06_220)]">
            {initial ? "Edit Service" : "Add New Service"}
          </h2>
          <button onClick={onCancel} className="p-2 rounded-full hover:bg-[oklch(0.94_0.02_205)]">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-[oklch(0.5_0.03_215)] mb-1">Category</label>
          ...
          <div>
            <label className="block text-xs font-medium text-[oklch(0.5_0.03_215)] mb-1">Description</label>
            <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
              rows={3} className="w-full border border-[oklch(0.88_0.01_215)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[oklch(0.55_0.12_195)] resize-none" />
          </div>
          {[
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm text-[oklch(0.35_0.03_220)]">Active (visible on site)</span>
          </label>
        </div>
  ...
  return (
    <div className="flex min-h-screen bg-[oklch(0.97_0.01_210)]">
      ...
      <main className="flex-1 p-8 overflow-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="font-serif text-3xl font-bold text-[oklch(0.18_0.06_220)]">Services</h1>
            <p className="text-[oklch(0.5_0.03_215)] mt-1">Manage your IV drip treatments and pricing</p>
          </div>
            ...
            <div key={svc.id} className="glass-card rounded-2xl p-5">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="font-semibold text-[oklch(0.22_0.07_220)]">{svc.name}</div>
                    <div className="text-xs text-[oklch(0.55_0.12_195)]">{svc.category} · {svc.tag}</div>
                  </div>
                </div>
              ...
              <p className="text-xs text-[oklch(0.5_0.03_215)] line-clamp-2 mb-3">{svc.description}</p>
              <div className="flex items-center justify-between">
                <span className="text-xl font-bold text-[oklch(0.38_0.09_220)]">£{parseFloat(svc.price).toFixed(0)}</span>
                <div className="flex gap-2">
                  <button onClick={() => setEditing(svc)} className="p-1.5 rounded-lg hover:bg-[oklch(0.94_0.02_205)] text-[oklch(0.38_0.09_220)]">

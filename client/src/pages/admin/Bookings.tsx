  ...
  return (
    <div className="flex min-h-screen bg-[oklch(0.97_0.01_210)]">
      ...
      <main className="flex-1 p-8 overflow-auto">
        <div className="mb-6">
          <h1 className="font-serif text-3xl font-bold text-[oklch(0.18_0.05_60)]">Bookings</h1>
          <p className="text-[oklch(0.50_0.03_65)] mt-1">Manage all client appointments and consent forms</p>
        </div>
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          ...
          <div className="relative">
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
              className="appearance-none pl-4 pr-8 py-2.5 border border-[oklch(0.87_0.025_78)] rounded-xl bg-white text-sm focus:outline-none focus:border-[oklch(0.52_0.10_75)] text-[oklch(0.35_0.04_65)]">
              ...
              <thead>
                <tr className="border-b border-[oklch(0.87_0.025_78)] bg-[oklch(0.96_0.03_78)]">
                  {["Client", "Service", "Date & Time", "Status", "Actions"].map(h => (
                    <th key={h} className="text-left px-5 py-3 font-semibold text-[oklch(0.45_0.03_65)] text-xs uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  [...Array(5)].map((_, i) => (
                    <tr key={i} className="border-b border-[oklch(0.92_0.01_215)]">
                      {[...Array(5)].map((_, j) => (
                        <td key={j} className="px-5 py-4"><div className="h-4 bg-[oklch(0.92_0.03_78)] rounded animate-pulse" /></td>
                      ))}
                    </tr>
                  ))
                ) : bookings.length === 0 ? (
                  <tr><td colSpan={5} className="text-center py-12 text-[oklch(0.58_0.03_65)]">No bookings found.</td></tr>
                ) : (
                  bookings.map((b: any) => (
                    <tr key={b.id} className="border-b border-[oklch(0.92_0.01_215)] hover:bg-[oklch(0.97_0.005_215)] transition-colors">
                      ...
                      <td className="px-5 py-4 text-[oklch(0.35_0.04_65)]">{b.serviceName}</td>
                      <td className="px-5 py-4">
                        <div className="text-[oklch(0.35_0.04_65)]">{b.bookingDate}</div>
